...
// 卡片日期
export function postFormatDateString(dateString: string) {
  const options: Intl.DateTimeFormatOptions = {
    year: "numeric",
    month: "short",
    day: "numeric",
...
export const postMultiFormatDateString = (timestamp: string = ""): string => {
  const timestampNum = Math.round(new Date(timestamp).getTime() / 1000);
  const date: Date = new Date(timestampNum * 1000);
  const now: Date = new Date();
  ...
  switch (true) {
    case Math.floor(diffInDays) >= 6:
      return postFormatDateString(timestamp);
    case Math.floor(diffInDays) >= 1 && diffInDays < 6:
      return `${Math.floor(diffInDays)} 天前`;
    case Math.floor(diffInHours) >= 1:
      return `${Math.floor(diffInHours)} 小時前`;
